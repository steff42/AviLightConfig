...
public class TestTelegramSeparator
{
  private class TelegramSeparatorTester implements DataProcessor
  {
    ...
    {
      super();
      this.toTest = new TelegramSeparationProcessor( this,
    ...
    public void processData(byte[] buffer, int length)
    {
      ...
      else
      {
        ...
        Assert.assertTrue( equals, "Expected: " + HexUtils.toHex( next )
  ...
  public void testSimpleTelegram()
  {
    ...
    for (int b = 0; b < 255; b++)
    {
      ...
      {
        // Escape
        send = new byte[] { 2, 27, (byte) b, 3 };
        expected = new byte[][] { { (byte) b } };
      }
      else
      {
        send = new byte[] { 2, (byte) b, 3 };
        expected = new byte[][] { { (byte) b } };
        ;
      }
  ...
  public void testSplittedTelegram()
  {
    ...
    tester.testProcessData( new byte[] { 2, 27 }, 2 );
    tester.setExpectNext( new byte[][] { { 3 } } );
    tester.testProcessData( new byte[] { 3, 3 }, 2 );

    tester.setExpectNext( null );
    tester.testProcessData( new byte[] { 2 }, 1 );
    tester.testProcessData( new byte[] { 25 }, 1 );
    tester.setExpectNext( new byte[][] { { 25 } } );
    tester.testProcessData( new byte[] { 3 }, 1 );
    Assert.assertNotNull( tester.getLastTelegram(), "Nothing received" );
  }
  ...
  public void testMultipleTelegram()
  {
    ...
    tester.setExpectNext( new byte[][] { { 7 }, { 8 } } );
    tester.testProcessData( new byte[] { 2, 7, 3, 2, 8, 3 }, 6 );
    Assert.assertEquals( tester.getNextIndex(), 2, "Telegram count mismatch" );

    tester.setExpectNext( new byte[][] { { 127, (byte) 128, (byte) 129 },
        { 2, 3, 27, 5 } } );
    tester.testProcessData(
        new byte[] { 2, 127, (byte) 128, (byte) 129, 3, 2 }, 6 );
    tester.testProcessData( new byte[] { 2, 27, 3, 27, 27, 5, 3 }, 7 );
    Assert.assertEquals( tester.getNextIndex(), 2, "Telegram count mismatch" );
  }
}
